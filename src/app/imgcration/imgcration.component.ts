...
export class ImgcrationComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if (this.loggedinuser == null) {
      ...
      console.log( ' this.courseContents .length : '+this.courseContents .length);
      this.startAutoSave();
    }
  }
  ngOnDestroy() {
  ...
  changeVideoSource(contentOrder: number) {

    const currCourse = this.courseContents.find(d => d.order === contentOrder) as icoursecontent;
    if (currCourse) {
      ...
      console.error("No course content found for the selected order:", this.selectedOrder);
    }
